...
Current task: the P3s [done]. **Every finding in every cold read in `qa/` is
now closed**, at every severity — P0 through P3 — with the one exception noted
against 6.5 #23 below, which cannot be fixed without breaking `migrate deploy`
and is documented instead. The outstanding blockers are unchanged and are
