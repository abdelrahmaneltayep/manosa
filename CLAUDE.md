...
2. **Every query is scoped to one shop.** Reaching another shop's row by id must read as not found — never as a leak. Four things hold that line, and knowing which is which is the point: the Prisma extension in `app/lib/tenant/` merges `shop` into every `where` and stamps it on every `create` for a scoped **model** operation, and throws outside a scope; **raw SQL** carries no model, so it cannot be filtered and is instead refused outside a scope, with `shop` written into each of the five call sites' own `WHERE`; a **to-one relation and a nested create inside an `update`** are reached by neither, and are held by the composite `(shop, parentId)` foreign key on every owned relation; and `withoutShopScope()` is the only deliberate escape — it takes a written reason, and there are four of them. `tests/integration/tenant-relations.test.ts` probes every scoped model that owns a relation and fails when a new one appears without a probe.
